...
function Home() {
  ...
  const [showHistory, setShowHistory] = useState(false); // 👈 state for history
  ...
  return (
    <div className="w-full h-[100vh] bg-gradient-to-t from-[black] to-[#02023d] flex justify-center items-center flex-col gap-[15px] overflow-hidden overflow-y-auto relative">
      ...
      {/* Large Screen History Toggle */}
      <button
        className="min-w-[150px] h-[60px] mt-[30px] text-black font-semibold bg-white absolute top-[180px] right-[20px] rounded-full cursor-pointer text-[19px] hidden lg:block"
        onClick={() => setShowHistory(!showHistory)}
      >
        {showHistory ? "Hide History" : "Show History"}
      </button>

      {/* Large Screen History Box */}
      {showHistory && (
        <div className="hidden lg:flex flex-col absolute top-[260px] right-[20px] w-[300px] h-[400px] bg-[#0a0a2a] rounded-2xl p-[15px] overflow-y-auto shadow-lg">
          <h1 className="text-white font-semibold text-[19px] mb-3">History</h1>
          <div className="flex flex-col gap-[10px]">
            {userData.history?.map((his, index) => (
              <div
                key={index}
                className="text-gray-200 text-[16px] truncate hover:text-white cursor-pointer"
              >
                {his}
              </div>
            ))}
